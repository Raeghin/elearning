...
$pdf = block_ccv_generate_pdf(get_string('report_d', 'block_ccv'));
$output = '';
...
function generate_header($coursename, $groupname, $fromdate, $todate)
{
	...
	$tabledata = array();
	$tabledata[] = (object) ['title' => get_string ( 'report_name' , 'block_ccv' ), 'value' => get_string ( 'report_d', 'block_ccv' )];
	$tabledata[] = (object) ['title' => '&nbsp;', 'value' => get_string ( 'report_d_text', 'block_ccv' )];
	...
	return block_ccv_get_page_header(get_string ( 'report_d', 'block_ccv' ), $tabledata);
}
...
function generate_body($userrecords, $coursename = null, $groupname, $courseid)
{
	...
	$output .= '<table class="tg">
	  			<tr>
    				...
    				<th width="100px" class="tg-9hbo">'.get_string ( 'not_finished_header', 'block_ccv' ).'</th>';
	if($groupname)
...
$pdf->Output ( $course->fullname . ' - ' . get_string ( 'report_d', 'block_ccv' ) . '.pdf', 'D' );
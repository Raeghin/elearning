...
function attendanceregister_add_to_log($register, $cmId, $action, $userId = null, $groupId = null) {
    ...
    switch ($action) {
        case ATTENDANCEREGISTER_ACTION_RECALCULATE:
			$event = \mod_attendanceregister\event\session_updated::create(
				array(
					...
					'other' => array(	//for legacy logging
						'action' => 'recalc',
						'urlaction'=>'recalc'
					)
				)
        ...
        default:		
			...
			$event = \mod_attendanceregister\event\course_module_viewed::create(
				array(
					'objectid' => $register->id,
					'context' => $context,
					'other'=>$other	//for legacy logging
				)
			);
    }	
    // Add Log Entry    
